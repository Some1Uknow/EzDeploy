...
async function init() {
    ...
    if (!fs.existsSync(outDirPath)) {
        const errorMsg = 'Output directory does not exist. Git clone may have failed.'
    ...
    p.on('close', async function (code) {
        ...
        if (!distFolderPath) {
            const errorMsg = 'No build output directory found (dist, build, out, or public)'
            console.error(errorMsg)
            publishLog(`error: ${errorMsg}`)
            return
        }
        ...
        publishLog(`Using build output directory: ${path.basename(distFolderPath)}`)
        ...
        try {
            ...
            for (const file of distFolderContents) {
                ...
                if (fs.lstatSync(filePath).isDirectory()) continue;                console.log('uploading', filePath)
